...
Extracts OCPBUGS references from git commit messages. Jira enrichment
(status, labels, release_action) is done via MCP Atlassian OAuth in
the enrich_ocpbugs.py script.
"""
...
def query_resolved_bugs(version, branch=None, since_version=None,
                        since_commit=None):
    """Scan commits for OCPBUGS references and output them for MCP enrichment.
