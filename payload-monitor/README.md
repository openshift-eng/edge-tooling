...
+----+-----+ +--------+ +--------+ +----+-----+ +----+----+ +----------+
     ...
     +---------------+------------------+-------------+-----------+
             ...
             +--------+----------+
                      |
             +--------v----------+      +---------------------+
             |  HTML Dashboard   |<-----| Doctor Fragments    |
             |  (Payload Health, |      | (MicroShift CI,     |
             |   MicroShift CI,  |      |  LVMS CI - optional)|
             |   LVMS CI tabs)   |      +---------------------+
             +-------------------+
```
...
- **MicroShift CI tab** (opt-in): Embedded MicroShift CI doctor report with sub-tabs for Periodics, Pull Requests, Bugs, and Image Health
- **LVMS CI tab** (opt-in): Embedded LVMS CI doctor report with sub-tabs for Periodics, Pull Requests, Bugs, and Image Health
...
The dashboard can include MicroShift CI and LVMS CI doctor reports as additional tabs. Doctor reports are generated by the shared `create-report.py` script in `plugins/shared/scripts/`, which analyzes CI periodic job failures, rebase PR status, bug correlations, and container image health.
